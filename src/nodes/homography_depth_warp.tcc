...
void homography_depth_warp<Depth>::process(flow::node_job& job) {	
	...
	std::fill(out.begin(), out.end(), null_depth_);
	...
	for(std::ptrdiff_t x = 0; x < image_shape[1]; ++x) {
		...
		if(out_cam.image_span().includes(dest_pix_coord)) {
			Depth& output_dest_pix_depth = out.at(dest_pix_coord);
			if(output_dest_pix_depth == null_depth_ || dest_pix_depth > output_dest_pix_depth)
				output_dest_pix_depth = source_pix_depth;
		}
	}
}
