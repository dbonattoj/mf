...
std::array<real, 2> projection_view_frustum::depth_components_(const projection_depth_parameters& dparam) {
	real z_diff = dparam.z_far - dparam.z_near;
	std::array<real, 2> components;
	if(dparam.depth_range == projection_depth_parameters::signed_normal) {
...
(real near_width, real near_height, const projection_depth_parameters& dparam) {
...
(angle near_width, angle near_height, const projection_depth_parameters& dparam) {
	real z_diff = dparam.z_far - dparam.z_near;
	auto dcomp = depth_components_(dparam);

	Eigen_mat4 mat; mat <<
		std::tan(half_pi - near_width), 0, 0, 0,
		0, std::tan(half_pi - near_height), 0, 0,
		0.0, 0.0, dcomp[0], dcomp[1],
		0, 0, -1.0, 0;
...
(angle near_width, real aspect_ratio, const projection_depth_parameters& dparam) {
...
(angle near_height, real aspect_ratio, const projection_depth_parameters& dparam) {
	real h = std::tan(half_pi - near_height);
	real w = h * aspect_ratio;
	auto dcomp = depth_components_(dparam);

	Eigen_mat4 projection_matrix;
	projection_matrix <<
		w, 0, 0, 0,
		0, h, 0, 0,
		0.0, 0.0, dcomp[0], dcomp[1],
...
(distance_pair near_x, distance_pair near_y, const projection_depth_parameters& dparam) {
	real two_nz = 2.0 * dparam.z_near;
...
(angle_pair near_x, angle_pair near_y, const projection_depth_parameters& dparam) {
...
float projection_view_frustum::near_z() const {
	return matrix(2, 3) / (matrix(2, 2) - 1);
}


float projection_view_frustum::far_z() const {
	return matrix(2, 3) / (matrix(2, 2) + 1);
}
