...
void event::notify(counter_type count) {
	int fd = static_cast<int>(handle_);
	std::uint64_t cnt = static_cast<std::uint64_t>(count);
...
event::counter_type event::wait() {
	int fd = static_cast<int>(handle_);
	std::uint64_t cnt = 0;
	auto len = ::read(fd, static_cast<void*>(&cnt), sizeof(std::uint64_t));
	if(len == sizeof(std::uint64_t)) return cnt;
	else throw std::system_error(errno, std::system_category(), "eventfd: read failed");
}


event_wait_result detail::wait_any(event** begin, event** end) {
	// prepare array of pollfd for poll function
	std::size_t n = end - begin;
	std::vector<::pollfd> fds(n);
	std::transform(begin, end, fds.begin(), [](event* ev) -> ::pollfd {
		::pollfd fd;
		fd.fd = static_cast<int>(ev->handle());
		fd.events = POLLIN;
		fd.revents = 0;
		return fd;	
	});
	...
	event::counter_type cnt = received_event->wait();
	return event_wait_result { *received_event, cnt };
}
