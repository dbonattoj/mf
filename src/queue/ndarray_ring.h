...
protected:
	frame_properties frame_properties_() const noexcept {
		frame_properties frame_prop;
		frame_prop.length = frame_shape_.product() * sizeof(Elem);
		frame_prop.alignment = alignof(Elem);
		return frame_prop;
	}
...
template<std::size_t Dim, typename Elem>
class ndarray_ring : public ndarray_ring_base<Dim, Elem>, public ring {
	using base = ndarray_ring_base<Dim, Elem>;
...
public:
	...
	ndarray_ring(const frame_shape_type& frame_shape, time_unit duration) :
		base(frame_shape),
		ring(base::frame_properties_(), duration) { }
...
template<std::size_t Dim, typename Elem>
class ndarray_timed_ring : public ndarray_ring_base<Dim, Elem>, public timed_ring {
	using base = ndarray_ring_base<Dim, Elem>;
...
public:
	...
	ndarray_timed_ring(const frame_shape_type& frame_shape, time_unit duration) :
		base(frame_shape),
		timed_ring(base::frame_properties_(), duration) { }
...
template<std::size_t Dim, typename Elem>
class ndarray_shared_ring : public ndarray_ring_base<Dim, Elem>, public shared_ring {
	using base = ndarray_ring_base<Dim, Elem>;
...
public:
	...
	ndarray_shared_ring(const frame_shape_type& frame_shape, std::size_t capacity, bool seekable, time_unit end_time = -1) :
		base(frame_shape),
		shared_ring(base::frame_properties_(), capacity, seekable, end_time) { }
