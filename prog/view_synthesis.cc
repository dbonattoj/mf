...
int main(int argc, const char* argv[]) {
	...
	auto camera_at_time = [&](time_unit t) {
		const camera_type& leftmost_cam = data.visuals.front().camera;
		const camera_type& rightmost_cam = data.visuals.back().camera;
		...
		real k = (1.0 + std::cos(pi * t / 25.0)*0.7) / 2.0;
	...
	for(const input_visual& vis : data.visuals) {
		...
		auto& di_warper = graph.add_node<node::homography_depth_warp<std::uint8_t>>(0.0);
	...
	auto& sink_converter = graph.add_node<node::color_converter<std::uint8_t, rgb_color>>();
	sink_converter.input.connect(di_warper.destination_depth_output);
...
/*
		...
		auto& warper = graph.add_node<node::reverse_homography_warp<rgba_color, std::uint8_t>>(rgba_color::null());
		warper.source_image_input.connect(im_converter.output);
		warper.destination_depth_input.connect(di_warper.destination_depth_output);		
