...
TEST_CASE("ndarray_view", "[ndarray_view]") {
	...
	SECTION("basics") {
		// row major strides
		REQUIRE( (ndarray_view<1, int>::row_major_strides(ndsize<1>(10))) == (ndptrdiff<1>{l}) );
		REQUIRE( (ndarray_view<2, int>::row_major_strides(ndsize<2>(10, 10))) == (ndptrdiff<2>{10*l, l}) );
		REQUIRE( (ndarray_view<3, int>::row_major_strides(ndsize<3>(4, 3, 2))) == (ndptrdiff<3>{3*2*l, 2*l, l}) );
		...
		REQUIRE(a1.strides() == a1.row_major_strides(shp));
		
		// non-default strides
		ndptrdiff<3> str(4,2,1);
		ndarray_view<3, int> a2(raw.data(), shp, ndarray_order::row_major, str);
		REQUIRE(a2.strides() == str);
		REQUIRE(a2.size() == 4*3*4);

		// comparison and assignment
		ndarray_view<3, int> a3(raw.data() + 13, shp, ndarray_order::row_major, str);
		REQUIRE(a1 == a1);
		REQUIRE_FALSE(a1 != a1);
		REQUIRE_FALSE(a1 == a3);
		REQUIRE_FALSE(a3 == a1);
		...
		REQUIRE(a3.strides() == a3.row_major_strides(shp));
		REQUIRE(a3 == a1);
		REQUIRE(a1 == a3);
		REQUIRE_FALSE(a1 != a3);
		REQUIRE_FALSE(a3 != a1);
